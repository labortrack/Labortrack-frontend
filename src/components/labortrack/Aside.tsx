import * as React from "react";
import { cn } from "../ui/utils";
import { DisplayXS, CaptionMD, LabelXS } from "./Typography";
import {
    LayoutDashboard,
    Users,
    UserCog,
    HardHat,
    Calendar,
    ClipboardCheck,
    ClipboardList,
    UserX,
    FileText,
    BookOpen,
    Sparkles,
    Building2,
    ChevronRight,
    ShieldCheck,
    CalendarOff,
    CalendarDays,
    Briefcase,
    LogOut,
    X,
} from "lucide-react";

export interface UserInfo {
    name: string;
    role: string;
    email?: string;
    initials: string;
    bg?: string;
}

export interface AsideProps {
    activeItem?: string;
    onItemClick?: (itemId: string) => void;
    className?: string;
    userInfo?: UserInfo;
    userRole?: string;
    hiddenItems?: string[];
    onLogout?: () => void;
    // Control Mobile Drawer
    isMobileOpen?: boolean;
    onCloseMobile?: () => void;
}

interface NavItem {
    id: string;
    label: string;
    icon: React.ComponentType<{ className?: string }>;
    badge?: string;
    roles?: string[];
}

export const navigationItems: NavItem[] = [
    { id: "dashboard", label: "Dashboard", icon: LayoutDashboard },
    { id: "legajos", label: "Legajos", icon: Users, roles: ["ROLE_ADMIN", "ROLE_RRHH"] },
    { id: "usuarios", label: "Usuarios", icon: UserCog, roles: ["ROLE_ADMIN"] },
    { id: "obras", label: "Obras", icon: HardHat, roles: ["ROLE_ADMIN", "ROLE_RRHH", "ROLE_CAPATAZ"] },
    { id: "jornadas", label: "Jornadas", icon: Calendar, roles: ["ROLE_ADMIN", "ROLE_RRHH", "ROLE_CAPATAZ"] },
    { id: "asistencia", label: "Asistencias", icon: ClipboardCheck, roles: ["ROLE_ADMIN", "ROLE_RRHH", "ROLE_CAPATAZ"] },
    { id: "ausencias", label: "Ausencias", icon: UserX, roles: ["ROLE_ADMIN", "ROLE_RRHH"] },
    { id: "recibos", label: "Recibos", icon: FileText, roles: ["ROLE_ADMIN", "ROLE_RRHH"] },
    { id: "documentacion", label: "Documentación", icon: BookOpen, roles: ["ROLE_ADMIN", "ROLE_RRHH"] },
    { id: "ia", label: "IA", icon: Sparkles, badge: "NUEVO", roles: ["ROLE_ADMIN"] },
    { id: "higiene", label: "Higiene y Seguridad", icon: ShieldCheck, roles: ["ROLE_ADMIN", "ROLE_RRHH"] },
    { id: "mi-empresa", label: "Mi empresa", icon: Briefcase, roles: ["ROLE_ADMIN"] },
    // Operario
    { id: "mis-asistencias", label: "Mis asistencias", icon: ClipboardList, roles: ["ROLE_OPERARIO"] },
    { id: "mis-ausencias", label: "Mis ausencias", icon: CalendarOff, roles: ["ROLE_OPERARIO"] },
    { id: "mis-jornadas", label: "Mis jornadas", icon: CalendarDays, roles: ["ROLE_OPERARIO"] },
    { id: "mis-recibos", label: "Mis recibos", icon: FileText, roles: ["ROLE_OPERARIO"] },
];

export function Aside({
    activeItem = "dashboard",
    onItemClick,
    className,
    userInfo,
    userRole,
    hiddenItems = [],
    onLogout,
    isMobileOpen = false,
    onCloseMobile,
}: AsideProps) {
    const [isCollapsed, setIsCollapsed] = React.useState(false);

    const visibleItems = navigationItems.filter((item) => {
        if (hiddenItems.includes(item.id)) return false;
        if (userRole && item.roles && !item.roles.includes(userRole)) return false;
        return true;
    });

    const handleItemClick = (id: string) => {
        onItemClick?.(id);
        onCloseMobile?.(); // Cierra el drawer automáticamente en móviles
    };

    return (
        <>
            {/* 1. Backdrop oscurecido para Mobile/Tablet */}
            {isMobileOpen && (
                <div
                    onClick={onCloseMobile}
                    className="fixed inset-0 z-40 bg-black/40 backdrop-blur-xs transition-opacity lg:hidden"
                    aria-hidden="true"
                />
            )}

            {/* 2. Contenedor del Aside */}
            <aside
                className={cn(
                    // Base & Posicionamiento
                    "fixed top-0 bottom-0 left-0 z-50 flex flex-col bg-[#f0eded] border-r border-[#e8e8e8] select-none h-screen transition-all duration-300 ease-in-out",
                    // Mobile: Drawer deslizable
                    isMobileOpen ? "translate-x-0 shadow-2xl w-72" : "-translate-x-full",
                    // Desktop (lg+): Estático y visible siempre
                    "lg:translate-x-0 lg:static lg:z-auto lg:shadow-none",
                    // Control de colapso en Desktop
                    isCollapsed ? "lg:w-20" : "lg:w-64",
                    className
                )}
            >
                {/* Header con Logo y botón de cierre en Mobile */}
                <div className="px-6 py-6 border-b border-[#e8e8e8] flex items-center justify-between">
                    <div className="flex items-center gap-3 min-w-0">
                        <div className="w-10 h-10 bg-[#0036a4] rounded-lg flex items-center justify-center flex-shrink-0 shadow-xs">
                            <Building2 className="w-6 h-6 text-white stroke-[1.75]" />
                        </div>
                        {(!isCollapsed || isMobileOpen) && (
                            <div className="flex flex-col min-w-0">
                                <DisplayXS className="text-[#1c1b1b] truncate">
                                    LaborTrack
                                </DisplayXS>
                                <CaptionMD className="text-[#636363] text-[11px] truncate">
                                    Control de Personal
                                </CaptionMD>
                            </div>
                        )}
                    </div>

                    {/* Botón cerrar en Mobile */}
                    <button
                        type="button"
                        onClick={onCloseMobile}
                        className="p-1.5 text-[#636363] hover:text-[#1c1b1b] hover:bg-[#e8e8e8] rounded-md transition-colors lg:hidden"
                        title="Cerrar menú"
                    >
                        <X className="w-5 h-5" />
                    </button>
                </div>

                {/* Lista de navegación scrolleable */}
                <nav className="flex-1 px-3 py-4 overflow-y-auto overflow-x-hidden space-y-1">
                    {visibleItems.map((item) => {
                        const Icon = item.icon;
                        const isActive = activeItem === item.id;

                        return (
                            <button
                                key={item.id}
                                type="button"
                                onClick={() => handleItemClick(item.id)}
                                className={cn(
                                    "w-full flex items-center gap-3 px-3.5 py-3 rounded-[0.25rem] transition-all group relative",
                                    isActive
                                        ? "bg-[#0036a4] text-white shadow-xs"
                                        : "text-[#636363] hover:bg-[#e8e8e8] hover:text-[#1c1b1b]"
                                )}
                                title={isCollapsed && !isMobileOpen ? item.label : undefined}
                            >
                                {/* Riel Naranja Institucional activo */}
                                {isActive && (
                                    <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-[#ff9d2e] rounded-r-full -ml-3" />
                                )}

                                <Icon
                                    className={cn(
                                        "w-5 h-5 flex-shrink-0 transition-colors",
                                        isActive ? "text-white" : "text-[#636363] group-hover:text-[#0036a4]"
                                    )}
                                />

                                {(!isCollapsed || isMobileOpen) && (
                                    <>
                                        <span
                                            className={cn(
                                                "flex-1 text-left text-[14px] leading-[20px] font-medium truncate",
                                                isActive ? "text-white" : "text-[#1c1b1b]"
                                            )}
                                        >
                                            {item.label}
                                        </span>

                                        {item.badge && (
                                            <span className="px-2 py-0.5 bg-[#ff9d2e] text-white text-[10px] leading-[14px] font-bold rounded-full uppercase tracking-wider">
                                                {item.badge}
                                            </span>
                                        )}

                                        {isActive && (
                                            <ChevronRight className="w-4 h-4 text-white opacity-75" />
                                        )}
                                    </>
                                )}
                            </button>
                        );
                    })}
                </nav>

                {/* Botón Contraer (Solo visible en Desktop) */}
                <div className="hidden lg:block px-3 py-3 border-t border-[#e8e8e8]">
                    <button
                        type="button"
                        onClick={() => setIsCollapsed(!isCollapsed)}
                        className="w-full flex items-center justify-center gap-2 px-3 py-2 text-[#636363] hover:text-[#0036a4] hover:bg-[#e8e8e8] rounded-[0.25rem] transition-colors"
                        title={isCollapsed ? "Expandir menú" : "Contraer menú"}
                    >
                        <div
                            className={cn(
                                "flex items-center gap-1.5 transition-transform duration-200",
                                isCollapsed && "rotate-180"
                            )}
                        >
                            <div className="flex gap-0.5">
                                <div className="w-1 h-4 bg-[#636363] rounded-full" />
                                <div className="w-1 h-4 bg-[#636363] rounded-full" />
                                {!isCollapsed && <div className="w-1 h-4 bg-[#636363] rounded-full" />}
                            </div>
                        </div>
                        {!isCollapsed && (
                            <LabelXS className="text-[#636363]">Contraer</LabelXS>
                        )}
                    </button>
                </div>

                {/* Tarjeta de Usuario & Logout */}
                {(!isCollapsed || isMobileOpen) && (
                    <div className="px-4 py-4 border-t border-[#e8e8e8] bg-[#e8e8e8]">
                        <div className="flex items-center gap-3">
                            <div
                                className="w-10 h-10 rounded-full flex items-center justify-center text-white text-[14px] font-bold shadow-xs flex-shrink-0"
                                style={{ backgroundColor: userInfo?.bg ?? "#0036a4" }}
                            >
                                {userInfo?.initials ?? "AD"}
                            </div>
                            <div className="flex-1 min-w-0">
                                <div className="text-[13px] font-bold text-[#1c1b1b] truncate">
                                    {userInfo?.name ?? "Usuario"}
                                </div>
                                <div className="text-[11px] text-[#636363] truncate">
                                    {userInfo?.role ?? "Operario"}
                                </div>
                            </div>
                            <button
                                type="button"
                                onClick={onLogout}
                                className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-[0.25rem] text-[#636363] transition-colors hover:bg-[#fdf0f0] hover:text-[#ba1a1a]"
                                title="Cerrar Sesión"
                            >
                                <LogOut className="h-4 w-4" />
                            </button>
                        </div>
                    </div>
                )}
            </aside>
        </>
    );
}