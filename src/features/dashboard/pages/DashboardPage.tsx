        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p className="flex items-center gap-1.5 text-[13px] font-medium text-[#636363] mt-1">
                        <User className="h-4 w-4 text-[#757575]" />
                        Sesión iniciada como: <strong className="text-[#1c1b1b]">{currentUserEmail}</strong>
                    </p>
                </div>
                ...
                <div className="flex items-center gap-3">
                    <button
                        ...
                        className="flex items-center gap-2 rounded-lg border border-[#cfd1d4] bg-white px-4 py-2 text-[13px] font-semibold text-[#1c1b1b] transition-colors hover:bg-[#f7f7f8] disabled:opacity-50 shadow-2xs"
                    >
                        {isTestingHi ? (
                            <Loader2 className="h-4 w-4 animate-spin text-[#0036a4]" />
                        ) : (
                            <ShieldCheck className="h-4 w-4 text-[#0036a4]" />
                        )}
            ...
            {testMessage && (
                <div className="flex items-center justify-between rounded-lg border border-[#004c28]/20 bg-[#e6f4ee] px-4 py-3 text-[13px] font-semibold text-[#004c28]">
                    <div className="flex items-center gap-2">
                        ...
                        <span>
                            Respuesta del backend: <strong>{testMessage}</strong>
                        </span>
                    </div>
                    <button
                        type="button"
                        onClick={() => setTestMessage("")}
                        className="text-xs underline hover:text-[#0036a4] cursor-pointer"
                    >
                        Descartar
                    </button>
                </div>
            ...
            {/* Tabla de Usuarios */}
            <section className="rounded-xl border border-[#e8e8e8] bg-white shadow-2xs overflow-hidden">
                <div className="flex items-center justify-between border-b border-[#e8e8e8] p-5">
                    ...
                    >
                        <RefreshCw
                            className={`h-3.5 w-3.5 ${isLoadingUsers ? "animate-spin" : ""}`}
                        />
                        Actualizar lista
                    </button>
                </div>
