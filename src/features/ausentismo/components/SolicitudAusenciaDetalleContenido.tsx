...
import { CalendarDays, Clock, Hash, MessageSquare, Tags, Paperclip, History, Briefcase, Eye, type LucideIcon } from "lucide-react";
...
import { TipoSolicitudAusenciaDatos } from "./TipoSolicitudAusenciaDatos";
import { DocumentoVisorIntegrado } from "@/features/documentacion/components/DocumentoVisorIntegrado";
import type { DocumentoAusencia, SolicitudAusenciaDetalle } from "../types/solicitudAusencia.types";

export function SolicitudAusenciaDetalleContenido({ solicitud }: { solicitud: SolicitudAusenciaDetalle }) {
...
function DocumentoVisor({ documento, onClose }: { documento: DocumentoAusencia; onClose: () => void }) {
  ...
  return <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}><DialogContent className="max-h-[90dvh] overflow-y-auto sm:max-w-4xl">
    <DialogHeader><DialogTitle className="flex items-start gap-2 break-words"><Paperclip className="size-5 shrink-0 text-primary" aria-hidden="true" />{documento.nombreArchivoOriginal || documento.nombre}</DialogTitle><DialogDescription>Documentación adjunta a la solicitud de ausencia.</DialogDescription></DialogHeader>
