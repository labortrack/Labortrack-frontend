...
export function CreateUserDialog({
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    try {
      ...
      setSubmitError(
        normalizeApiError(error, "No se pudo crear el usuario.").message,
      );
    }
  });
...
export function EditUserDialog({
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    setSubmitError(undefined);
    try {
      await mutation.mutateAsync({ id: user.idUsuario, payload: values });
      toast.success("Usuario actualizado correctamente.");
      handleOpenChange(false);
    } catch (error) {
      setSubmitError(
        normalizeApiError(error, "No se pudo modificar el usuario.").message,
      );
    }
  });
  return (
      ...
      <DialogContent>
        ...
        <form onSubmit={onSubmit} className="space-y-4" noValidate>
          ...
          <p className="text-xs text-foreground-muted">
            El contrato actual del backend permite modificar únicamente nombre y
            apellido.
          </p>
          <DialogFooter>
            <Button
  ...
  const onSubmit = handleSubmit(async ({ motivo }) => {
    ...
    try {
      await mutation.mutateAsync({
        id: user.idUsuario,
        payload: { motivo, fechaBaja: localDate() },
      });
      toast.success("Usuario dado de baja correctamente.");
      handleOpenChange(false);
    } catch (error) {
      setSubmitError(
        normalizeApiError(error, "No se pudo dar de baja el usuario.").message,
      );
    }
  });
  return (
        ...
        <DialogHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <DialogDescription>
                Esta acción deshabilitará el acceso al sistema.
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        <form onSubmit={onSubmit} className="space-y-4" noValidate>
          ...
          >
            <Textarea
              ...
              placeholder="Indicá la razón de la baja..."
              {...register("motivo")}
            />
          </FormField>
          <DialogFooter>
